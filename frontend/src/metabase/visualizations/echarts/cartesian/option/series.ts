  ...
  return {
    ...
    symbol: (value, params) => {
      ...
      if (xcontrolShowCustom) {
        if (value && params && params.seriesId) {
          const xCardId = params.seriesId.substring(0, params.seriesId.indexOf(":"));
          const valDatapoint = Number(value[params.seriesId]);
          const valCL = Number(value[xCardId+":CL"]);
          const valSTDDEV = Number(value[xCardId+":STDDEV"]);
          if (isValidNumber(valDatapoint) && isValidNumber(valCL) && isValidNumber(valSTDDEV)) {
            const valUCL = (valCL + valSTDDEV * 3);
            const valLCL = (valCL - valSTDDEV * 3) > 0? (valCL - valSTDDEV * 3) : 0;
